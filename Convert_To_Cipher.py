...
def getChaoticSequence(x,image):#it takes image as input
    im = Image.open(image)  # Can be many different formats.
    image_size = im.size #Get the width and hight of the image for iterating over
    print("Image Size : ",image_size)
    image_matrix = []
    xn = random.random() #seed value
    for width in range(int(image_size[0])):
        row = []
        for height in range(int(image_size[1])):
            try:
...
def DeciOfBin(binary):
    ...
    return decimal
###############################################GLOBAL Variables######################################
file_name="Abhra_int.bmp"
cipher_file_name="Abhra_cipher.bmp"
...
#    print(MatrixToImage(size,im))
    ...
    im_key=getChaoticSequence(x,file_name) #Opens with line 259 
#    print(im_key)
