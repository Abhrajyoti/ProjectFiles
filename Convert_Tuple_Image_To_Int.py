...
def getImageMatrix_tuple(image):#it takes the image having tuples as pixels
    ...
    for width in range(int(image_size[0])):
        ...
        for height in range(int(image_size[1])):
            try:
                row.append(DeciOfBin(binarySequence((val_tuple(pix[width,height])))))
            except:
                row=[DeciOfBin(binarySequence((val_tuple(pix[width,height]))))]
        try:
            image_matrix.append(row)
        except:
            image_matrix = [row]
#    print(MatrixToImage(size,image_matrix))
...
def getImageMatrix_list(image):#it takes the image as lists
    ...
    for width in range(int(image_size[0])):
        ...
        for height in range(int(image_size[1])):
            try:
                row.append(DeciOfBin(binarySequence((pix[width,height]))))
            except:
                row=[DeciOfBin(binarySequence((pix[width,height])))]
        try:
            image_matrix.append(row)
        except:
            image_matrix = [row]
...
###############################################GLOBAL Variables######################################
file_name="Abhra.jpg"
cipher_file_name="Abhra_int.bmp"
...
#    im_key=getChaoticSequence(file_name) #Opens with line 259 
#    print(im_key)
#    im=getImageMatrix_tuple(im)
#    print(im)
#    print(LogMapEncrypt(size,getCipher_im(im,im_key))) #Gets the image into cipher image
